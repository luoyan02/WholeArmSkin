...
robot = dict()
#import robot.json
with open('/home/emprise/wholearm_ws/wholearm_ws/robot.json') as json_file:
...
def callback(data):
    ...
    for e in reversed(range(len(link_names))):
       ...
       # tran.transform.translation.z = tran.transform.translation.z/home/emprise/wholearm_ws/wholearm_ws/robot.json
        ...
        for i in range(len(robot[link_names[e]])):  
            ...
            #q_new = tf.transformations.quaternion_multiply(quaternion, tf.transformations.quaternion_from_euler(-np.pi/2, 0, 0))
            q_new = quaternion
