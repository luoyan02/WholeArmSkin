...
data = pickle.load(open("data_collection_squish_diel_single.pickle", "rb"))
...
model_l = {'m': m, 'c': c}
with open('fitted_model_squish_diel_single.pickle', 'wb') as handle:
...
a0, a1, a2, a3 = poly3_regression(x, y)
model_p3 = {'a0': a0, 'a1': a1, 'a2': a2, 'a3': a3}
with open('fitted_poly3_model_squish_diel_single.pickle', 'wb') as handle:
    pickle.dump(model_p3, handle, protocol=pickle.HIGHEST_PROTOCOL)

c0, c1, c2, c3, c4, c5 = poly5_regression(x, y)
model_p5 = {'c0': c0, 'c1': c1, 'c2': c2, 'c3': c3, 'c4': c4, 'c5': c5}
with open('fitted_poly5_model_squish_diel_single.pickle', 'wb') as handle:
...
ax1.plot(x, y, 'o' 'c', label='Original data', markersize=2)
ax1.plot(x, x * m + c, 'r', label='Fitted line - linear')
# ax1.plot(x, log_func(x, a, b, k), 'b--', dashes=(5, 10), label='Fitted line - log linear')
ax1.plot(x, a3 + a2*x + a1*x**2 + a0*x**3, 'g', label='Fitted line - poly3')
ax1.plot(x, c5 + c4*x + c3*x**2 + c2*x**3 + c1*x**4 + c0*x**5, 'm',  label='Fitted line - poly5')
ax1.set_xlabel('Skin readings')
ax1.set_ylabel('Forque readings')
ax1.set_title('Forque vs. Skin data')
ax1.legend()

ratio = (max(x)-min(x))/(min(y)-max(y))

ax2.plot(np.array(data['skin_time']), x, 'c', label='Skin data')
ax2.plot(np.array(data['skin_time']), y*ratio, 'r', label='Forque data (scaled by %f)' %ratio)
ax2.set_xlabel('Time')
