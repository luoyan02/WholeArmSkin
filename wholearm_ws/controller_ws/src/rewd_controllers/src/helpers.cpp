...
Eigen::MatrixXd ExtendedJointPosition::pseudoinverse(const Eigen::MatrixXd& mat, double eps)
{
  ...
  else
  {
    ...
    // double tolerance = eps * std::max(mat.cols(), mat.rows()) *svd.singularValues().array().abs()(0);
...
Eigen::MatrixXd ExtendedJointPosition::pseudoinverse2(const Eigen::MatrixXd& mat, double eps)
{
  if (mat.rows() == mat.cols() && mat.determinant() > eps)
    return mat.inverse();
