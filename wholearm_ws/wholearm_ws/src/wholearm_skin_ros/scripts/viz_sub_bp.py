...
pos = [[13.964,44.504,42.851, 89.033, 0, -90], [-13.978, 44.373, 42.945, 89.037, 0, -90],[15.138,45.408,93.34, 86.53,0,-90],
[-15.254,45.412, 93.575, 86.514, 0, -90], [15,47.612,138.1,90.7, 0, -90], [-14.998, 47.613, 137.402, 90.655, 0, -90],
...
def callback(data):
    try:
        tran = tf2_buffer.lookup_transform("base_link", "shoulder_link", rospy.Time())
    except (tf2_ros.LookupException, tf2_ros.ConnectivityException, tf2_ros.ExtrapolationException):
        return
    x,y,z= tran.transform.translation.x*1000, tran.transform.translation.y*1000, tran.transform.translation.z *1000
    ...
    arr = np.asarray([x,y,z,roll,pitch,yaw])
    for i in range(len(pos)):    
        true_pos[i] = np.asarray(pos[i]) + arr
        if i == 0:
            print("transformation: ")
            print(arr)
            print("taxel location: ")
            print(true_pos[0])
    ...
    for i, value in enumerate(data.data):
        ...
        marker.scale.y = .0075
        marker.scale.z = .0075
        marker.pose.position.x = true_pos[i][0] /1000
        marker.pose.position.y = true_pos[i][1] /1000
        marker.pose.position.z = true_pos[i][2] /1000
        quaternion = tf.transformations.quaternion_from_euler(np.radians(true_pos[i][3]), np.radians(true_pos[i][4]), np.radians(true_pos[i][5]))
