...
class DataCollection:
    def __init__(self) -> None:
        ...
        self.skin_sub = message_filters.Subscriber("/calibration", Float32MultiArray)
    ...
    def sync_callback(self, ft_msg, skin_msg):
        ...
        self.filter_count += 1
        skin_data = self.filter(skin_msg.data[taxel] - self.tare_value)

        if not self.taring:
            self.skin_history.append(skin_data)
        elif self.filter_count > 300:
            print("FT: ", ft_msg.wrench.force.z, " Skin: ", skin_data)
            self.store_data(ft_msg.wrench.force.z, skin_data, ft_msg.header.stamp.to_sec())
    ...
    def save_data(self):
        rospy.loginfo('Saving inference data') 
        with open('inference_data_l6t3_0217.pickle', 'wb') as handle: 
