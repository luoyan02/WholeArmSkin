...
//configure a measurement
uint8_t FDC1004::configureMeasurementDifferential(uint8_t measurement, uint8_t poschannel, uint8_t negchannel)
{
    //Verify data
    if (!FDC1004_IS_MEAS(measurement) || !FDC1004_IS_CHANNEL(poschannel) || !FDC1004_IS_CHANNEL(negchannel) || (poschannel == negchannel)) {
        Serial.println("bad configuration");
        return 1;
    }
    ...
    //build 16 bit configuration
    uint16_t configuration_data;
    configuration_data = ((uint16_t)poschannel) << 13; //CHA
    configuration_data |= ((uint16_t)negchannel) << 10; //CHB
    write16(MEAS_CONFIG[measurement], configuration_data);
    return 0;
}
 ...
 */
uint8_t FDC1004::measureChannel(uint8_t poschannel, uint8_t negchannel, uint8_t capdac, uint16_t * value, uint8_t isdiff)
{
  uint8_t measurement = poschannel; //4 measurement configs, 4 channels, seems fair
  if (isdiff) {
      if (configureMeasurementDifferential(measurement, poschannel, negchannel)) return 1;
  }
  else {
      if (configureMeasurementSingle(measurement, poschannel, capdac)) return 1;
  }
